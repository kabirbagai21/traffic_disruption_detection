# SUPPOSED TO BE IN THIS DIRECTORY: RAM_path/ram/models
...
class RAM_plus(nn.Module):
    ...
    def generate_tag_openset(self,
        ...
        return tag_output
    def generate_tag_openset_prob(self,
                 image,
                 tag_input=None,
                 return_probs=False,  # <-- Optional toggle
                 ):
        ...
        logits = self.fc(tagging_embed[0]).squeeze(-1)
        probs = torch.sigmoid(logits)  # shape: [bs, num_class]

        probs[:, self.delete_tag_index] = 0  # zero out deleted tags

        # Return a list of dicts: one per image, mapping tag -> probability
        prob_output = []
        for b in range(bs):
            prob_dict = {tag: float(probs[b, i]) for i, tag in enumerate(self.tag_list)}
            prob_output.append(prob_dict)
